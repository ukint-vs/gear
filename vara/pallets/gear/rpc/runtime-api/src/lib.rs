...
sp_api::decl_runtime_apis! {
    ...
    pub trait GearApi {
        ...
        /// Returns `Ok(None)` when either the code or the section is absent.
        /// Returns `Err(message)` on invalid section name UTF-8 or stored-WASM
        /// parse failure (latter is defensive — codes are validated on upload).
        fn read_code_custom_section(code_id: H256, section_name: Vec<u8>) -> Result<Option<Vec<u8>>, Vec<u8>>;
